...
export class VistaCitas {
  constructor(gestor) {
    this.gestor = gestor;
    this.svcCitasDecorado = new DisponibilidadDecorador(new ServicioCitas(gestor, 1, window.appState?.citaRepo));
  }
    ...
    container.innerHTML = `
      <div class="panel">
        ...
        <form id="formCita" class="form-grid">
          ...
          <label>Duración (minutos, máx. 120)<input id="duracion" type="number" min="1" max="120" required /></label>
  ...
  async cargarTabla() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '';
    const citas = await this.svcCitasDecorado.obtenerTodas();
    citas.forEach((c) => {
  ...
  async reprogramarCita(id) {
    ...
    if (!hora) return;
    const res = await this.svcCitasDecorado.reprogramarCita(id, fecha, hora);
    alert(res.mensaje);
    if (res.exito) this.cargarTabla();
  }
  ...
  async guardar(e) {
    ...
    const res = await this.svcCitasDecorado.registrarCita(pacId, serv, cons, fecha, hora, dur);
    alert(res.mensaje);
    if (res.exito) {
      this.cargarTabla();
    }

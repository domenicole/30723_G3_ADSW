...
export class VistaPacientes {
      ...
      <div class="dashboard-shell">
        ...
        <main class="main-panel">
          <section id="pacientes" class="view-card active">
            ...
            <table>
              <thead><tr><th>Cédula</th><th>Nombre</th><th>Teléfono</th><th>Estado</th></tr></thead>
  ...
  cargarTabla() {
    ...
    this.svcDecorado.obtenerPacientes().forEach((p) => {
      ...
      fila.innerHTML = `<td>${p.cedula}</td><td>${p.nombre}</td><td>${p.telefono}</td><td>${p.estado}</td>`;
  ...
  guardar(e) {
    ...
    const res = this.svcDecorado.registrarPaciente(cedula, nombre, telefono, fecha, domicilio, ocupacion, estadoCivil, '');
    if (res.exito) {
      alert(res.mensaje);
      this.render();
    }
